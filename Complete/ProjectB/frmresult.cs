...
namespace ProjectB
{
    public partial class frmresult : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            while (reader.Read())
            {
                if (reader.GetString(1) == txt_assess.Text)
                {
                    ...
                    reader2 = Database_Connection.get_instance().Getdata(String.Format("SELECT StudentId,AssessmentComponentId, RubricMeasurementId,EvaluationDate, Name,AC.TotalMarks, MeasurementLevel From Assessment A JOIN  AssessmentComponent AC ON A.Id =AC.AssessmentId  JOIN StudentResult SR ON SR.AssessmentComponentId=AC.Id JOIN RubricLevel RL ON SR.RubricMeasurementId = RL.Id WHERE AssessmentId='{0}'", temp));
        ...
        {
            exportpdf(assessmentresult, "assessmentpdfs");
        }
    }
}

    ...
    public partial class addrubric : Form
    {
        ...
        private void Viewcl_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            if (Viewrub.Columns[e.ColumnIndex].Name == "Delete")
            {
                ...
                string tempr = edit.Cells[0].Value.ToString();
                curr2 = Int32.Parse(tempr);
                MessageBox.Show("Are you sure you want to delete(It will delete all the related Rubrib Levels and Assessment Component)?");
                ...
                string listasscom = string.Format("SELECT * FROM AssessmentComponent WHERE RubricId='{0}'", curr2);
                if (Database_Connection.get_instance().Listofassessmentcomp(listasscom) != null)
                {
                    string cmdasscomp = string.Format("DELETE FROM AssessmentComponent WHERE RubricId='{0}'", curr2);
